...
extension TextDrawing {
  ...
  private func availableTextRect(
    yPosition: CGFloat,
    preferences: TextPreferences,
    topOffset: CGFloat,
    radius: CGFloat,
    sliceDegree: CGFloat,
    margins: SFWConfiguration.Margins
  ) -> [CGRect] {
    ...
    let maxLines = preferences.numberOfLines == 0 ? Int(
      (maxHeight / (preferences.font.pointSize + preferences.spacing)).rounded(.up)
    ) : preferences.numberOfLines
...
extension TextDrawing {
  ...
  func drawCurved(
    ...
    radius: CGFloat,
    sliceDegree: CGFloat,
    contextPositionCorrectionOffsetDegree: CGFloat,
    margins: SFWConfiguration.Margins
  ) -> CGFloat {
    ...
    let textRects = availableTextRect(
      yPosition: yPosition,
    ...
    let height = preferences.font.pointSize * CGFloat(
      multilineString.count
    ) + preferences.spacing * max(
      0, CGFloat(multilineString.count - 1)
    )
  ...
  func drawVertical(
    ...
    let maxWidth: CGFloat = .circularSegmentHeight(radius: radius - preferences.verticalOffset - topOffset,
                                                   from: sliceDegree)
    ...
    for line in 1..<Int(maxLinesInSlice) {
      ...
      let _bottomRadiusOffset = max(margins.bottom, .radius(circularSegmentHeight: _maxCircularSegmentHeight,
                                                            from: sliceDegree))
      ...
      let _nextBottomRadiusOffset = max(margins.bottom, .radius(
        circularSegmentHeight: _nextMaxCircularSegmentHeight,
        from: sliceDegree)
      )
    ...
    for index in 0..<availableTextRects.count {
      ...
      multilineString = text.split(
        font: preferences.font,
        lineWidths: textRects.map({ min(preferences.maxWidth, $0.width) }),
        lineBreak: preferences.lineBreakMode
      )
