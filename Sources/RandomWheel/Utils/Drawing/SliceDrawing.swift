...
extension SliceDrawing {
  ...
  var margins: SFWConfiguration.Margins {
    ...
    margins.top += (self.preferences?.circlePreferences.strokeWidth ?? 0) / 2
    margins.left += (self.preferences?.circlePreferences.strokeWidth ?? 0)
    margins.right += (self.preferences?.circlePreferences.strokeWidth ?? 0)
    margins.bottom += (self.preferences?.circlePreferences.strokeWidth ?? 0) / 2
    return margins
  }
...
extension SliceDrawing {
  ...
  ///   - start: start degree
  ///   - end: end degree
  func drawSlice(withIndex index: Int,
                 in context: CGContext,
                 forSlice slice: Slice,
                 rotation: CGFloat,
                 start: CGFloat,
                 end: CGFloat) {
    //// Context setup
    ...
    slice.contents.enumerated().forEach { (contentIndex, element) in
      switch element {
      case let .text(text, preferences):
        topOffset += prepareDraw(text: text,
                                 ...
                                 index: index,
                                 topOffset: topOffset)
      case let .assetImage(imageName, preferences):
        ...
        self.drawImage(in: context,
                       ...
                       margins: margins)
        topOffset += preferences.preferredSize.height + preferences.verticalOffset
      case let .image(image, preferences):
        self.drawImage(in: context,
                       image: image,
                       preferences: preferences,
                       rotation: rotation,
  ...
  ///   - rotation: rotation degree
  ///   - index: index
  private func drawPath(in context: CGContext,
                        backgroundColor: SFWColor?,
                        backgroundGradientColor: SFGradientColor?,
                        backgroundImage: SFWImage?,
                        start: CGFloat,
                        and end: CGFloat,
                        rotation: CGFloat,
                        index: Int) {
    ...
    if pathBackgroundColor == nil {
      ...
      case let .some(.evenOddColors(evenColor, oddColor)):
    ...
    if let gradientType = backgroundGradientColor?.type,
      ...
      case .linear:
        ...
        let start = direction == .vertical ?
        CGPoint(x: path.boundingBox.minX, y: path.boundingBox.midY) :
        CGPoint(x: path.boundingBox.minX, y: path.boundingBox.minY)
        
        let end = direction == .vertical ?
        CGPoint(x: path.boundingBox.maxX, y: path.boundingBox.midY) :
        CGPoint(x: path.boundingBox.maxX, y: path.boundingBox.maxX)
  ...
  private func draw(backgroundImage: SFWImage, in context: CGContext, clipPath: CGPath) {
    ...
    let aspectFillSize = CGSize.aspectFill(
      aspectRatio: backgroundImage.size,
      minimumSize: CGSize(width: radius, height: circularSegmentHeight)
    )
    ...
    case .some(.bottom):
      ...
      backgroundImage.draw(
        at: position,
        from: NSRect(x: 0, y: 0, width: rectangle.width, height: rectangle.height),
  ...
  func prepareDraw(text: String,
                   in context: CGContext,
                   preferences: TextPreferences,
                   rotation: CGFloat,
                   index: Int,
                   topOffset: CGFloat) -> CGFloat {
