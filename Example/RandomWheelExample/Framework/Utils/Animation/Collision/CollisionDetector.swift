...
final class CollisionDetector {
  ...
  func prepareWithContinuousAnimation(sliceDegree: CGFloat,
    ...
    continuousCollisionCalculator.calculateCollisionInterval(
      sliceDegree: sliceDegree,
      rotationDegreeOffset: rotationDegreeOffset,
      fullRotationDegree: fullRotationDegree,
      speed: speed,
  ...
  func prepare(sliceDegree: CGFloat,
               rotationDegree: CGFloat,
               rotationDegreeOffset: CGFloat,
               animationDuration: CFTimeInterval) {
    reset()
    
    collisionCalculator.calculateCollisionDegrees(
      sliceDegree: sliceDegree,
      rotationDegreeOffset: rotationDegreeOffset,
  ...
  func start() {
    ...
    timer = Timer.scheduledTimer(
      timeInterval: 0.01,
      arget: self,
      selector: #selector(timerUpdate(timer:)),
      userInfo: nil,
      repeats: true
    )
#else
  ...
  private func timerUpdate(timer: Timer) {
    continuousCollisionCalculator.calculateCollisionsIfNeeded(
      timestamp: CACurrentMediaTime(),
      onCollision: { [weak self] progress in
    ...
    collisionCalculator.calculateCollisionsIfNeeded(
      layerRotationZ: layerRotationZ,
      onCollision: { [weak self] progress in
        self?.onCollision?(progress)
      }
    )
