  ...
  func maxDuration(sublayersCount: NSInteger) -> CFTimeInterval {
    ...
    if let groupAnim = self as? CAAnimationGroup {
      for subAnim in groupAnim.animations! as [CAAnimation] {
        ...
        duration = CGFloat(subAnim.beginTime) + (
          subAnim.autoreverses ? CGFloat(subAnim.duration) : CGFloat(0)
        ) + delay + CGFloat(subAnim.duration) + CGFloat(repeatCountDuration)
...
extension Sequence where Element == CAAnimation {
  var maxDuration: CFTimeInterval {
    var maxDuration: CGFloat = 0
    for anim in self {
      maxDuration = Swift.max(
        CGFloat(
          anim.beginTime + anim.duration
        ) * CGFloat(
          anim.repeatCount == 0 ? 1.0 : anim.repeatCount
        ) * (
          anim.autoreverses ? 2.0 : 1.0
        ), maxDuration
      )
    }
