...
public final class SwiftFortuneWheel: SFWControl {
...
#if !os(macOS)
    pinImageViewAnimator.movePinIfNeeded(
      collisionEffect: self.pinImageViewCollisionEffect,
      position: self.configuration?.pinPreferences?.position
    )
#endif
...
public extension SwiftFortuneWheel {
  ...
  func startRotationAnimation(
    ...
    fullRotationsCount: Int = 13,
    animationDuration: CFTimeInterval = 5.000,
  ...
  func startRotationAnimation(
    finishIndex: Int,
    continuousRotationTime: Int,
    continuousRotationSpeed: CGFloat = 4,
    _ completion: ((Bool) -> Void)?) {
    let _index = finishIndex < self.slices.count ? finishIndex : self.slices.count - 1
...
public extension SwiftFortuneWheel {
  ...
  func startAnimating(
    finishIndex: Int,
    rotationOffset: CGFloat,
    fullRotationsUntilFinish: Int = 13,
    animationDuration: CFTimeInterval = 5.000,
    _ completion: ((Bool) -> Void)?
  ) {
    self.startRotationAnimation(
      ...
      rotationOffset: rotationOffset,
      fullRotationsCount: fullRotationsUntilFinish,
      animationDuration: animationDuration,
      completion
    )
  }
