...
final class WheelView: UIView {
  ...
  private func addWheelLayer() {
    ...
    // MARK: - I don't know how to fix clipping problem on macOS
    // MARK: - if you have a better solution, I will be happy to know
    let circleWidth = preferences?.circlePreferences.strokeWidth ?? 0
    let frame = NSRect(
      x: self.bounds.origin.x + circleWidth / 2,
      y: self.bounds.origin.y + circleWidth / 2,
      width: self.bounds.width - circleWidth,
      height: self.bounds.height - circleWidth
    )
